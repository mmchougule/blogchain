...
func TestShowHelp(t *testing.T) {
	net, objs := networkWithHelpObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowHelp(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetHelpResponse
...
func TestListHelp(t *testing.T) {
	net, objs := networkWithHelpObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListHelp(), args)
			require.NoError(t, err)
			var resp types.QueryAllHelpResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
